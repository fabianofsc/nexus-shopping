...
class CustomerMigrationContractTest {
    ...
    fun `customer seed should create ten customers with contacts and addresses`() {
        ...
        DriverManager.getConnection(jdbcUrl, "sa", "").use { connection ->
            ...
            assertEquals(10, countRows(connection, "customers WHERE document_type = 'CPF'"))
            assertEquals(1, countRows(connection, "customers WHERE name = 'Benjamin Bryan Duarte' AND document = '10521980232'"))
            assertEquals(1, countRows(connection, "customers WHERE name = 'Cláudia Elaine Eloá Galvão' AND document = '06075013083'"))
            assertEquals(1, countRows(connection, "customers WHERE name = 'Maitê Yasmin Cardoso' AND document = '08330170985'"))
